...
output_dir = f"lgbm_results_{timestamp}"
...
# === 6. Training function với LGBM ===
def train_lgbm(X_train, y_train, X_test, seed):
...
def run_experiment(X_train, y_train, X_test, y_test, name, output_dir):
    ...
    for run, seed in enumerate([42, 43, 44], start=1):
        y_pred, y_proba = train_lgbm(X_train, y_train, X_test, seed)
    ...
    df_raw.to_csv(os.path.join(output_dir, f"lgbm_metrics_{name}_raw.csv"), index=False)
    ...
    df_summary.to_csv(os.path.join(output_dir, f"lgbm_metrics_{name}_summary.csv"))
    print(f"✅ Đã lưu summary cho {name} → {output_dir}/lgbm_metrics_{name}_summary.csv")
    return df_summary
...
df_summary_all.to_csv(os.path.join(output_dir, "lgbm_metrics_all_summary.csv"))
print(f"\n📊 Trung bình ± SD trên 3 lần chạy (LightGBM):")
