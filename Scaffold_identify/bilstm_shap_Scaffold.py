...
def main(num_runs=3):
    ...
    df = pd.read_csv("capsule_x_train.csv").dropna(subset=['canonical_smiles', 'Toxicity Value'])
    ...
    y_array = df['Toxicity Value'].astype(int).values
    ...
    for run_index in range(num_runs):
        ...
        background = X_df.sample(n=min(200, len(X_df)), random_state=run_index)
        ...
        # === Scaffold SHAP scores ===
        for i, scaffold in enumerate(background_scaffold):
            shap_score = shap_values[i].mean()  # Trung bình toàn vector SHAP
            all_scaffold_shap.append((scaffold, shap_score))

    # === Tổng hợp kết quả scaffold SHAP ===
    ...
    print(f"✅ Saved: {output_dir}/scaffold_shap_summary.csv")

    # === Plot top 20 scaffold SHAP ===
    top_n = 20
    plt.figure(figsize=(10, 6))
    plt.barh(df_summary["scaffold"][:top_n][::-1], df_summary["mean_shap"][:top_n][::-1])
