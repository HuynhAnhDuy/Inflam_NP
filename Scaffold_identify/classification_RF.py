...
from sklearn.metrics import (
    ...
    roc_auc_score, precision_score, f1_score, precision_recall_curve,
...
# === 6. Chạy lặp huấn luyện RF ===
NUM_RUNS = 3
raw_results = []

for run in range(NUM_RUNS):
    seed = 42 + run
    model = RandomForestClassifier(class_weight='balanced', random_state=seed, n_estimators=500)
...
# === 7. Lưu kết quả từng lần
df_raw = pd.DataFrame(raw_results)
df_raw.to_csv("rf_metrics_ecfp+scaffold_raw.csv", index=False)
print("✅ Đã lưu kết quả từng lần chạy → rf_metrics_ecfp+scaffold_raw.csv")

# === 8. Lưu summary theo hàng = Scaffold, cột = metric
metrics_cols = [
    "Accuracy", "Balanced Accuracy", "AUROC", "AUPRC",
    "MCC", "Precision", "Sensitivity", "Specificity"
]

df_summary_stats = df_raw[metrics_cols].agg(['mean', 'std']).T
df_summary_stats["Scaffold"] = df_summary_stats["mean"].round(3).astype(str) + " ± " + df_summary_stats["std"].round(3).astype(str)

# Đặt hàng là "Scaffold", cột là từng metric
df_summary = df_summary_stats[["Scaffold"]].T
df_summary.index = ["Scaffold"]

df_summary.to_csv("rf_metrics_ecfp+scaffold_summary.csv")
print("✅ Đã lưu summary → rf_metrics_ecfp+scaffold_summary.csv")

# === 9. In kết quả ra terminal
