...
# ==== 4. Save SHAP values + expected + importance ====
def save_shap_results(shap_values, expected_value, feature_names, run_index, output_dir):
    shap_df = pd.DataFrame(shap_values, columns=feature_names)
    shap_df.to_csv(f"{output_dir}/shap_values_run{run_index}.csv", index=False)

    with open(f"{output_dir}/expected_value_run{run_index}.csv", "w") as f:
    ...
    importance_df.to_csv(f"{output_dir}/feature_importance_run{run_index}.csv", index=False)

# ==== 5. Main SHAP analysis pipeline ====
def main(num_runs=5):
    set_seed()
    ...
    for run_index in range(num_runs):
        ...
        shap_values = np.array(shap_values).squeeze()  # ép về 2D nếu có shape (200, 2048, 1)


        # === Save SHAP-related results ===
        save_shap_results(shap_values, expected_value, feature_names, run_index, output_dir)
        x_explain.to_csv(f"{output_dir}/x_explain_run{run_index}.csv", index=False)

        # === Aggregate for scaffold-based analysis ===
        ...
        all_shap_concat.append(pd.DataFrame(shap_values, columns=feature_names))
    print("✅ SHAP shape:", shap_values.shape)
    
    # === Concatenate 1000 SHAP samples
    df_concat = pd.concat(all_shap_concat, ignore_index=True)
    df_concat.to_csv(f"{output_dir}/shap_values_concat.csv", index=False)
    print(f"✅ Saved 1000-sample SHAP values → shap_values_concat.csv")
    ...
    print(f"✅ Saved scaffold SHAP summary → scaffold_shap_summary.csv")

    # === Plot: Top 20 scaffolds by SHAP mean
    top_n = 20
    plt.figure(figsize=(10, 6))
    color_map = df_summary["effect"][:top_n].map({"positive": "green", "negative": "red"})
    plt.barh(df_summary["scaffold"][:top_n][::-1], df_summary["mean_shap"][:top_n][::-1], color=color_map[::-1])
    plt.xlabel("Mean SHAP Value")
    plt.title("Top 20 Scaffold Impact (SHAP)")
    plt.tight_layout()
    plt.savefig(f"{output_dir}/top20_scaffold_shap.svg", format='svg', dpi=300)
    plt.close()
