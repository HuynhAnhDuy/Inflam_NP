...
# ====== 1) Load + merge all models (inner merge assumes same compounds across files) ======
model_dfs = []
model_names = []
...
# ====== 3) Compute probability ONLY among supporting (positive-vote) models ======
...
summary = (
    ...
    .agg(
        ...
        prob_support_mean=("prob_support_avg", "mean"),
        prob_support_median=("prob_support_avg", "median"),
        prob_support_std=("prob_support_avg", "std"),
        prob_support_q25=("prob_support_avg", lambda x: x.quantile(0.25)),
        prob_support_q75=("prob_support_avg", lambda x: x.quantile(0.75)),
    )
    .reindex(keep_levels)
    .reset_index()
)
...
summary = summary[[
    "consensus_level", "n_active", "n_compounds",
    "prob_support_mean", "prob_support_median",
    "prob_support_std", "prob_support_q25", "prob_support_q75"
]]
...
# Hide x tick labels on boxplot (vì table đã có header)
...
# --- Middle axis: only the xlabel text (tight, close to table) ---
ax_xlab.axis("off")
ax_xlab.text(
...
for cell in tbl.get_celld().values():
    cell.get_text().set_fontfamily("sans-serif")   # hoặc "Arial", "DejaVu Sans"
    cell.get_text().set_fontstyle("normal")        # "italic" nếu muốn
    cell.get_text().set_fontweight("normal")       # "bold" nếu muốn
...
# Header hàng (row labels) nằm ở cột -1
for r in range(len(table_rows)):
    cells[(r + 1, -1)].get_text().set_fontweight("bold")

# Header cột (col labels) nằm ở hàng 0, cột 0..n-1
