...
OUTPUT = "NPASS_candidate_consensus.csv"
KEY = "canonical_smiles"  # cột khóa chung
# ====================

dfs = []
probs = []  # lưu prob để tính trung bình
...
df_common = reduce(
    lambda left, right: pd.merge(left, right, on=KEY, how="inner"), dfs
)

# Ghép toàn bộ prob từ các file vào
for p in probs:
  df_common = pd.merge(df_common, p, on=KEY, how="inner")

# Tính trung bình các cột prob
...
print(
    f"Tìm thấy {len(result)} mẫu chung có y_pred=1 trong cả {len(FILES)} file"
)
